...
const Header: React.FC = () => {
  ...
  return (
    <header className="bg-gray-900/80 backdrop-blur-sm sticky top-0 z-50">
      ...
      <div className={`transition-all duration-300 ease-in-out md:hidden ${isMobileMenuOpen ? 'max-h-screen' : 'max-h-0'} overflow-hidden`}>
        <nav className="bg-gray-900/95 px-6 pb-4 pt-2">
            ...
            {navLinks.map((link) => (
              <li key={link.name}>
                <a
                  href={link.href}
                  onClick={(e) => handleNavClick(e, link.href)}
                  className="flex items-center text-gray-300 hover:text-cyan-400 transition-colors duration-300 py-2 rounded-md"
                >
                  {link.icon}
                  <span>{link.name}</span>
                </a>
